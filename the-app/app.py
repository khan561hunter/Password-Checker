...
if st.button("Check"):
    ...
    if any(char.isupper() for char in password):
        count += 1
        st.success("Password contains atleast one uppercase letter")
    else:
        st.error("Password must contain at least one uppercase letter")
    if any(char.islower() for char in password):
        count += 1
        st.success("Password contains atleast one lowercase letter")
    else:
        st.error("Password must contain at least one uppercase letter")
    if any(char.isdigit() for char in password):
        count += 1
        st.success("Password contains atleast one number")
    else:
        st.error("Password must contain at least one number")
    if any(char in "!@#$%^&*()" for char in password):
        count += 1
        st.success("Password contains atleast one special character")
    else:
